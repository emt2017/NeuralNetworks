...
%normalize data
Xtrain=FaceTrainCellArrayData;
Xtest=FaceTestCellArrayData;
...
for person = 1:40
    for img = 1:6
        for i = img+1:6
            ...
            ezrocG(count) = immse(y(1:hiddenSize/2,(img+(person-1)*6)),y(1:hiddenSize/2,(i+(person-1)*6)));
            count=count+1;
        end
    end
end
...
for person = 1:40
    for img = 1:6
        ...
        %compare personComp +1 then iterate 39times
            for imgComp = 1:6
                ezrocI(count) = immse(y(1:hiddenSize/2,(img+(person-1)*6)),y(1:hiddenSize/2,(imgComp+(personComp-1)*6)));
                count=count+1;
            end
        end
    end
