...
%save net??????????????????????????????????????????????????????????????????
%save (['\\kc.umkc.edu\kc-users\home\e\emt9q7\My Documents\MATLAB\NeuralNetsMiniProject4/SubjectIndependentNets/NetDelay_' num2str(BestInput) '_Node_' num2str(BestSize) '_Person_' num2str(personNum) '_fold_' num2str(crossNum) '.mat'], 'net');
...
ezroc3(Htotal);
%savefig(['\\kc.umkc.edu\kc-users\home\e\emt9q7\My Documents\MATLAB\NeuralNetsMiniProject4/6FoldROCCurves/FigureInput_' num2str(BestInput) '_Output_' num2str(BestOutput) '_Node_' num2str(BestSize) 'Person_' num2str(personNum) '.fig']);
