...
%save net??????????????????????????????????????????????????????????????????
%save (['\\kc.umkc.edu\kc-users\home\e\emt9q7\My Documents\MATLAB\NeuralNetsMiniProject3/SubjectSpecificNets/NetDelay_' num2str(BestDelay) '_Node_' num2str(BestSize) '_Person_' num2str(personNum) '_fold_' num2str(crossNum) '.mat'], 'net');
